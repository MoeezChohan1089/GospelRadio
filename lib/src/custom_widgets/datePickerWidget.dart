...
class DatePicker extends StatefulWidget {
  ...
  DatePicker(
    ...
    this.selectionColor = AppColors.customPinkColor,
    this.deactivatedColor = Colors.white,
...
class _DatePickerState extends State<DatePicker> {
  ...
  void initState() {
    ...
    this.selectedDayStyle = widget.dayTextStyle
        .copyWith(fontSize: 16, color: widget.selectedTextColor);

    this.deactivatedDateStyle =
        widget.dateTextStyle.copyWith(color: Colors.white);
    this.deactivatedMonthStyle =
        widget.monthTextStyle.copyWith(color: Colors.white);
    this.deactivatedDayStyle =
        widget.dayTextStyle.copyWith(color: Colors.white);
    WidgetsBinding.instance.addPostFrameCallback((_) {
      // Calculate the current date index
      int index = DateTime.now().difference(widget.startDate).inDays;

      // Scroll to the current date index
      _controller.animateTo(
        index * (widget.width + 10),
  ...
  Widget build(BuildContext context) {
    monthName = DateFormat('MMMM yyyy').format(_currentDate!).toString();
    return Column(
      children: [
        Padding(
          ...
          child: Align(
              ...
              child: Text(
                monthName,
                style: widget.context.text.titleMedium
                    ?.copyWith(fontSize: 18.sp, color: Colors.blue),
              )),
        ),
        SizedBox(
          height: 15,
        ...
        Container(
          ...
          child: ListView.builder(
            itemCount: DateTime.now().difference(widget.startDate).inDays + 1,
            ...
            itemBuilder: (context, index) {
              ...
              DateTime _date = widget.startDate.add(Duration(days: index+30));
...
//            print("Inside Inactive dates.");
                for (DateTime inactiveDate in widget.inactiveDates!) {
                  if (_compareDate(DateTime.now(), inactiveDate)) {
                    isDeactivated = true;
                    break;
                  }
                }
              ...
              // DateTime daysBefore = date.subtract(Duration(days: 30));
              // Check if this date is the one that is currently selected
              bool isSelected = _currentDate != null
                  ? _compareDate(date, _currentDate!)
                  : false;

              bool showMonth = date.day == 1;
              ...
              return Container(
                ...
                child: DateWidget(
                  ...
                  onDateSelected: (selectedDate) {
                    ...
                    // Update the month name if the selected date is in a different month
                    if (selectedDate.month != _currentDate?.month) {
                      setState(() {
                        monthName =
                            DateFormat("MMMM yyyy").format(selectedDate);
                        print("moeez chohan: $monthName");
